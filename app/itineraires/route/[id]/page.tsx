...
export default function Route (){
    ...
    useEffect(() => {
        // Extraction de l'ID d'itinéraire de l'URL
        ...
        const fetchItineraireData = async () => {
            try {
                ...
                // Ici on suppose que `data` contient une propriété `livraisons` qui est une liste d'IDs
                console.log(data);
                const articlesFetchPromises = data.commandes.map((livraisonId) =>
                    fetchLivraisonArticle(livraisonId.id)
    ...
    if (!itineraire) {
        return <div>Chargement...</div>; // Affichage en cas de chargement des données
    }
